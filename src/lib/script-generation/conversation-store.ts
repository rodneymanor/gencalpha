...
import type { ConversationContext, ConversationMessage, ScriptIteration, ActionType } from "./conversation-types";
  ...
  persist(
    (set, get) => ({
      ...
      addMessage: (sessionId: string, message: ConversationMessage) => {
        set((state) => {
          ...
          return {
            sessions: newSessions,
            activeConversation:
              state.activeConversation?.sessionId === sessionId ? updatedSession : state.activeConversation,
          };
        });
      },
      ...
      undoLastChange: (sessionId: string) => {
        ...
        const messagesWithSnapshots = session.history.filter((msg) => msg.scriptSnapshot).reverse();
