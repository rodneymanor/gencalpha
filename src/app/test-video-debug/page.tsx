...
export default function VideoDebugPage() {
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-4">🔧 Video Processing Debug Tool</h1>
        <p className="text-muted-foreground">
          Test each step of the video processing workflow to identify issues
        </p>
      </div>
        ...
        <CardContent>
          <div className="flex gap-4 items-end">
            <div className="flex-1">
              <label className="text-sm font-medium mb-2 block">Video URL to Test</label>
              <Input
                value={testUrl}
            ...
            <Button 
              onClick={testVideoScraping}
              disabled={isProcessing || !testUrl}
              className="px-8"
            >
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {results.map((result) => (
                  <div key={result.step} className="flex items-center justify-between p-3 border rounded-lg">
              ...
              <Tabs defaultValue="data" className="w-full">
                ...
                <TabsContent value="data">
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    {results.filter(r => r.data).map((result) => (
                      <div key={result.step} className="border rounded p-3">
                        <h4 className="font-semibold text-sm mb-2">{result.step}</h4>
                        <Textarea
                ...
                <TabsContent value="errors">
                  <div className="space-y-4">
                    {results.filter(r => r.error).map((result) => (
                      <div key={result.step} className="border border-red-200 rounded p-3 bg-red-50">
                        <h4 className="font-semibold text-sm mb-2 text-red-800">{result.step}</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Your logs show TikTok returns many "subtitleLinks" - we need to ensure we're selecting the actual video, not subtitle tracks.
              </p>
