...
export function VideoManagementPanel({
  ...
  const handleAddVideo = async () => {
    ...
    try {
      ...
      const result = await VideoCollectionService.addVideoToCollection(user.uid, selectedCollectionId, videoData);
      ...
      footer={
        <div className="flex items-center justify-between border-t border-neutral-200 bg-white px-6 py-4">
          <div className="flex items-center gap-2 text-sm text-neutral-500">
            ...
            <span>{isProcessing ? status.message : "Add video from TikTok or Instagram"}</span>
          </div>
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            ...
            />
            <div className="text-xs text-neutral-400">Choose which collection to add this video to</div>
          </div>
        </div>
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center justify-between">
              {urlError || (!validationResult.isValid && videoUrl) ? (
                ...
                <div className="text-xs text-neutral-400">Paste a TikTok or Instagram video URL</div>
              )}
        ...
        <div className="rounded-[var(--radius-card)] border border-neutral-200 bg-neutral-50 p-4 shadow-[var(--shadow-soft-drop)]">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="flex items-center gap-1">
              <div className="rounded-pill h-2 w-2 bg-neutral-900" />
              <span>TikTok Videos</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="rounded-pill bg-primary-500 h-2 w-2" />
        ...
        {/* Processing Status */}
        {isProcessing && (
          <div className="bg-primary-50 border-primary-200 rounded-[var(--radius-card)] border p-4 shadow-[var(--shadow-soft-drop)]">
            <div className="mb-2 flex items-center gap-2">
              <Clock className="text-primary-600 h-4 w-4 animate-pulse" />
              <span className="text-primary-900 text-sm font-medium">Processing Video</span>
            </div>
            <div className="text-primary-700 text-xs">{status.message}</div>
            <div className="mt-2 flex items-center gap-2">
              <div className="bg-primary-200 rounded-pill h-1 flex-1">
                <div className="bg-primary-600 rounded-pill h-1 animate-pulse" style={{ width: "60%" }} />
              </div>
              <span className="text-primary-600 text-xs">{status.label}</span>
            </div>
          </div>
        )}
        ...
        {processingStage === "complete" && (
          <div className="bg-success-50 border-success-200 rounded-[var(--radius-card)] border p-4 shadow-[var(--shadow-soft-drop)]">
            <div className="mb-2 flex items-center gap-2">
              <Check className="text-success-600 h-4 w-4" />
              <span className="text-success-900 text-sm font-medium">Video Added Successfully!</span>
            </div>
