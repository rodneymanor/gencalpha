...
export function useScriptsApi(): UseScriptsApiReturn {
  ...
  const createScript = useCallback(async (scriptData: CreateScriptRequest): Promise<Script | null> => {
    if (!user) {
  ...
  const updateScript = useCallback(async (id: string, updateData: UpdateScriptRequest): Promise<Script | null> => {
    if (!user) {
      setError('User not authenticated');
      return null;
    }
    ...
    try {
      ...
      if (data.success && data.script) {
        setScripts(prev => prev.map(script => 
          script.id === id ? data.script : script
        ));
        return data.script;
      } else {
