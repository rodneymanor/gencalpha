...
const defaultInsightsData = {
  ...
  videoCaption: "🔥 This changes everything! Here&apos;s what I learned about creating engaging content that actually converts. The secret isn&apos;t what you think... #ContentCreator #ViralTips #SocialMediaStrategy"
};
      ...
      <div className="space-y-6">
        <InsightsSection title="Hook Ideas" icon={Lightbulb}>
          <ul className="space-y-2">
            {defaultInsightsData.hookIdeas.map((idea, ideaIndex) => (
              <li key={`hook-${idea.slice(0, 20)}-${ideaIndex}`} className="text-sm text-muted-foreground flex items-start gap-2">
        ...
        <InsightsSection title="Content Ideas" icon={FileText}>
          <ul className="space-y-2">
            {defaultInsightsData.contentIdeas.map((idea, ideaIndex) => (
              <li key={`content-${idea.slice(0, 20)}-${ideaIndex}`} className="text-sm text-muted-foreground flex items-start gap-2">
                <span className="text-secondary font-medium mt-0.5">•</span>
                <span>{idea}</span>
              </li>
            ))}
        ...
        <InsightsSection title="Video Caption" icon={MessageSquare}>
          <div className="bg-muted/50 rounded-[var(--radius-button)] p-4">
            <p className="text-sm text-foreground">{defaultInsightsData.videoCaption}</p>
          </div>
        </InsightsSection>

        <InsightsSection title="Transcript" icon={VideoIcon}>
          <div className="bg-muted/50 rounded-[var(--radius-button)] p-4 max-h-48 overflow-y-auto">
            <div className="space-y-3 text-sm leading-relaxed">
              {video.transcript ? (
                video.transcript.split('\n').map((line: string, lineIndex: number) => (
                  <p key={`transcript-${line.slice(0, 15)}-${lineIndex}`} className="text-sm text-muted-foreground">
                    {line.trim()}
                  </p>
                ))
              ) : (
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Hey everyone, welcome back to my channel. Today I want to talk about something that&apos;s been on my mind lately - how we can create content that truly resonates with our audience. I&apos;ve been experimenting with different approaches, and I think I&apos;ve discovered something really interesting...
                </p>
              )}
            </div>
          </div>
