...
import { Calendar, Clock, Settings, Plus, Send, Save, Trash2, Edit3 } from "lucide-react";
    ...
    <div className={cn("w-full", className)}>
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-4 bg-muted/30 h-auto">
          <TabsTrigger 
            value="compose" 
            className="text-xs data-[state=active]:border-[#555864] data-[state=active]:bg-transparent px-1 py-2"
          >
            Compose
          </TabsTrigger>
          <TabsTrigger 
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <div className="flex items-center gap-4">
                  ...
                  <span>saved</span>
                  <span>✓</span>
                  {isThread && <Badge variant="secondary" className="text-xs">T</Badge>}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2 pt-2 border-t">
                <div className="flex items-center gap-2">
                  ...
                  <Button 
                    ...
                    onClick={postNow}
                    disabled={!content.trim() || isOverLimit}
                    className="bg-[#555864] hover:bg-[#555864]/90 flex-1"
                  >
                    Post now
                  </Button>
                </div>
                    ...
                    <DialogContent className="sm:max-w-md">
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <Select value={selectedPlatform} onValueChange={(value: "tiktok" | "instagram" | "youtube") => setSelectedPlatform(value)}>
                      ...
                      <DialogFooter>
                        ...
                        <Button size="sm" className="bg-[#555864] hover:bg-[#555864]/90">Save Settings</Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
                </div>
                <Button variant="ghost" size="sm" className="h-auto p-0 text-xs text-muted-foreground mt-1">
                  Edit queue
                </Button>
              </div>
            </CardContent>
          ...
          <div className="space-y-3">
            {draftScripts.length === 0 ? (
              ...
              draftScripts.map((script) => (
                <Card key={script.id} className="cursor-pointer hover:bg-muted/50">
                  <CardContent className="p-2">
                    <p className="text-xs text-foreground line-clamp-2 mb-2">{script.content}</p>
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <div className="flex items-center gap-1">
                        <span className="text-xs">{script.characterCount || script.content.length}/{characterLimit}</span>
                        {script.isThread && <Badge variant="secondary" className="text-xs h-4 px-1">T</Badge>}
                        {script.platform && <Badge variant="outline" className="text-xs h-4 px-1">{script.platform}</Badge>}
                      </div>
                      <div className="flex items-center gap-1">
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs">
                          {script.scheduledDate ? new Date(script.scheduledDate).toLocaleDateString('en-US', { 
                            month: 'short', 
                            day: 'numeric',
                            hour: '2-digit',
                            minute: '2-digit'
                  ...
                  <CardContent className="p-2">
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-1">
                        {script.isThread && <Badge variant="secondary" className="text-xs h-4 px-1">T</Badge>}
                        {script.platform && <Badge variant="outline" className="text-xs h-4 px-1">{script.platform}</Badge>}
                        <Badge variant="outline" className="text-xs h-4 px-1 bg-green-50">Sent</Badge>
                      </div>
                    </div>
                  </CardContent>
                </Card>
