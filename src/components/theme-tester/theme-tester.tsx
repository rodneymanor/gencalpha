...
import { generateColorScales, generateCSSVariables, themePresets, type BaseColors } from "@/lib/color-system";
    ...
    <div className="mx-auto max-w-7xl space-y-8 p-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Palette className="text-brand-500 h-6 w-6" />
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="bg-primary text-primary-foreground flex items-center gap-2 rounded-[var(--radius-button)] px-4 py-2 text-sm font-medium transition-all hover:opacity-90"
          >
      ...
      <div className="border-border bg-card rounded-[var(--radius-card)] border p-6">
        ...
        <div className="grid grid-cols-2 gap-6 lg:grid-cols-3">
          {Object.entries(baseColors).map(([key, value]) => (
            <div key={key}>
              <label className="text-muted-foreground mb-2 block text-sm font-medium capitalize">{key}</label>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  value={value}
                  onChange={(e) => handleColorChange(key as keyof BaseColors, e.target.value)}
                  className="border-border bg-background flex-1 rounded-[var(--radius-button)] border px-3 py-2 text-sm"
                />
                <button
                  onClick={() => copyToClipboard(value, key)}
                  className="hover:bg-accent rounded-[var(--radius-button)] p-2 transition-colors"
                >
                  {copied === key ? <Check className="text-success-500 h-4 w-4" /> : <Copy className="h-4 w-4" />}
                </button>
              </div>
            </div>
          ))}
          ...
          {Object.entries(themePresets).map(([key, preset]) => (
            <button
              ...
              className={`rounded-[var(--radius-button)] px-4 py-2 text-sm font-medium transition-all ${
                activePreset === key ? "bg-primary text-primary-foreground" : "bg-neutral-100 hover:bg-neutral-200"
              }`}
            >
              {preset.name}
            </button>
      ...
      <div className="border-border bg-card rounded-[var(--radius-card)] border p-6">
        ...
        <div className="space-y-6">
          {Object.entries(scales).map(([scaleName, scale]) => (
            <div key={scaleName}>
              <h3 className="text-muted-foreground mb-2 text-sm font-medium capitalize">{scaleName} Scale</h3>
              <div className="flex gap-1">
                {Object.entries(scale).map(([number, color]) => (
                  ...
                  >
                    <div
                      className="border-border h-12 w-full rounded-[var(--radius-button)] border transition-transform hover:scale-105"
                      style={{ backgroundColor: color }}
                    />
                    <span className="bg-foreground text-background absolute bottom-full left-1/2 mb-2 -translate-x-1/2 rounded px-2 py-1 text-xs opacity-0 transition-opacity group-hover:opacity-100">
                      {number}
                    </span>
                  </button>
                ))}
      ...
      {/* Live Preview */}
      <div
        className="theme-preview border-border rounded-[var(--radius-card)] border p-6"
      ...
      <div className="border-border bg-card rounded-[var(--radius-card)] border p-6">
        ...
        <div className="text-muted-foreground space-y-3 text-sm">
          <p>
            1. <strong>Adjust base colors</strong> using the color pickers or hex inputs
          </p>
          <p>
            2. <strong>Try presets</strong> for quick theme exploration
          </p>
          <p>
            3. <strong>Preview scales</strong> to see all generated variants
          </p>
          <p>
            4. <strong>Test in context</strong> with the live preview section
          </p>
          <p>
            5. <strong>Export config</strong> when you find a combination you like
          </p>
          <p className="bg-accent mt-4 rounded-[var(--radius-button)] p-3">
            💡 <strong>Tip:</strong> Changing one base color automatically generates a full 50-950 scale with consistent
            relationships
          </p>
        </div>
      </div>
