  ...
  return (
    <div className="relative h-full">
      <div
        ...
        className={`prose prose-neutral h-full w-full max-w-none p-4 text-neutral-700 outline-none focus:outline-none [&>*:first-child]:mt-0 ${className} `}
        style={{ minHeight: "200px" }}
        data-placeholder={localContent ? "" : placeholder}
      />

      {/* Placeholder */}
      {!localContent && <div className="pointer-events-none absolute top-4 left-4 text-neutral-400">{placeholder}</div>}

      {/* Slash Menu */}
      {showSlashMenu && (
        <div
          className="absolute z-50 min-w-[200px] rounded-[var(--radius-card)] border border-neutral-200 bg-white py-2 shadow-[var(--shadow-soft-drop)]"
        ...
        >
          {slashMenuItems.map((item) => (
            <button
              ...
              className="flex w-full items-center gap-3 px-3 py-2 text-left text-sm transition-colors duration-150 hover:bg-neutral-100"
            >
              <span className="w-6 text-center font-mono text-neutral-400">{item.icon}</span>
