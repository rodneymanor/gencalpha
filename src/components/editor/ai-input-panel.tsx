...
export function AIInputPanel({
  ...
  const executeAIAction = async (actionType: string, option?: string, customPrompt?: string) => {
    ...
    try {
      ...
      // Handle edit action
      if (actionType === "edit") {
        alert("Edit mode activated - content is now editable");
        onClose();
        return;
      }
  ...
  // Get contextual universal actions based on context
  const contextualUniversalActions = [...UNIVERSAL_ACTIONS];
  if (context === "ideas" || context === "notes") {
    contextualUniversalActions.push({
      key: "remix",
      label: "Remix",
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        >
          ...
          <div className="max-h-80 space-y-1 overflow-y-auto p-3">
            ...
            {contextualUniversalActions.map((action) => (
              <div key={action.key}>
                {action.hasSubmenu ? (
                  <DropdownMenu>
