...
export function useCredits(options: CreditsOptions = {}): [CreditsState, CreditsControls] {
  ...
  const refreshCredits = useCallback(async () => {
    ...
    try {
      const stats = await billingService.getUsageStats(user.uid, "free"); // Default to free for now
      updateState({
        available: stats.currentPeriod.creditsRemaining,
        used: stats.currentPeriod.creditsUsed,
        limit: stats.currentPeriod.creditsLimit,
      });
    } catch (error) {
  ...
  const deductCredits = useCallback(
    async (amount: number, operation: string): Promise<boolean> => {
      ...
      try {
        const result = await billingService.deductCredits(user.uid, operation as any, "free", {
          manualAmount: amount,
        });
        if (result.success) {
    ...
    [user, state.available, state.used, state.limit, updateState, onInsufficientCredits],
  );
  ...
  const refundCredits = useCallback(
    async (amount: number, operation: string): Promise<boolean> => {
      ...
      try {
        const result = await billingService.refundCredits(user.uid, amount, operation);
        if (result.success) {
          // Update local state immediately for better UX
    ...
    [user, state.available, state.used, state.limit, updateState],
  );
