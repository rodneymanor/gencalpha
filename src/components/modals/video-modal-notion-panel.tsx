...
import NotionPanelProperties, { type PageProperty } from '@/components/panels/notion/NotionPanelProperties';
...
export function VideoModalNotionPanel({
  video,
  videoInsights,
  notionData
}: VideoModalNotionPanelProps) {
  ...
  const defaultTab = availableTabs.includes('analysis') ? 'analysis' : 
                    availableTabs.includes('transcript') ? 'transcript' : 
        ...
        {Object.keys(filteredTabData).length > 0 ? (
          ...
          <div className="flex-1 flex items-center justify-center p-8">
            <div className="text-center">
              <h3 className="text-lg font-medium text-neutral-900 mb-2">No content available</h3>
              <p className="text-sm text-neutral-600">
                This video doesn't have additional insights to display.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="border-t border-neutral-200 bg-neutral-50 p-4">
          <div className="flex items-center justify-between text-sm text-neutral-600">
            <div className="flex items-center gap-4">
              {videoInsights.metadata.duration && (
                <span>Duration: {Math.round(videoInsights.metadata.duration)}s</span>
              )}
            ...
            <div className="text-xs">
              {video.addedAt && new Date(video.addedAt).toLocaleDateString()}
            </div>
          </div>
        </div>
      )}
