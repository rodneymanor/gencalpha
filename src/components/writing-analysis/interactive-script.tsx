...
const applyComplexityHighlighting = (text: string, components: ScriptComponent[]): string => {
  let result = text;
...
export function InteractiveScript({ script, onScriptUpdate, className = "", scriptAnalysis }: InteractiveScriptProps) {
  ...
  const handleApplySuggestion = (newText: string) => {
    ...
    const updatedSections = sections.map((section) =>
      section === selectedSection ? { ...section, content: newText } : section,
    );
  ...
  return (
    <>
      <div className={`prose text-foreground min-h-[300px] max-w-none leading-relaxed ${className}`}>
        {sections.map((section, index) => (
          <div key={`section-${section.type}-${index}`} className="mb-6">
            {/* Section Title */}
            <h4 className="text-foreground mt-6 mb-3 text-lg font-semibold first:mt-0">{section.title}:</h4>

            {/* Interactive Section Content */}
            <div
              ref={(el) => (sectionRefs.current[index] = el)}
              className={`relative cursor-pointer rounded-[var(--radius-button)] p-4 transition-all duration-200 ${hoveredSection === index ? `${getSectionColor(section.type)} shadow-sm` : "hover:bg-background-hover"} `}
            ...
            >
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html:
                    scriptAnalysis?.hasComponentAnalysis && scriptAnalysis.componentAnalysis?.components
        ...
        {/* Instructions */}
        <div className="bg-muted/50 border-border mt-8 rounded-[var(--radius-button)] border p-4">
