...
export function MetricsPanel({ video }: { video: VideoData | null }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
      {items.map((m) => (
        <div key={m.label} className="rounded-[var(--radius-button)] bg-card p-4 transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-soft-drop)]">
...
export function ContentPanel({ video }: { video: VideoData | null }) {
  return (
    <div className="space-y-6">
      <div className="rounded-[var(--radius-button)] bg-card p-3">
...
export function TranscriptPanel({ transcript, stats }: {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <div className="rounded-[var(--radius-button)] bg-card p-3">
          <div className="text-[12px] text-muted-foreground">Words</div>
          <div className="text-xl font-semibold">{wordCount ?? '—'}</div>
        </div>
        <div className="rounded-[var(--radius-button)] bg-card p-3">
          <div className="text-[12px] text-muted-foreground">Sentences</div>
          <div className="text-xl font-semibold">{sentenceCount ?? '—'}</div>
        </div>
        <div className="rounded-[var(--radius-button)] bg-card p-3">
          <div className="text-[12px] text-muted-foreground">Avg Words/Sentence</div>
          <div className="text-xl font-semibold">{avgWordsPerSentence ? avgWordsPerSentence.toFixed(1) : '—'}</div>
        </div>
        <div className="rounded-[var(--radius-button)] bg-card p-3">
          <div className="text-[12px] text-muted-foreground">Reading Level</div>
          <div className="text-xl font-semibold">{easeText ?? '—'}</div>
        </div>
      </div>
      <div className="rounded-[var(--radius-button)] bg-card p-4">
...
export function VoicePanel({ analysis, isAnalyzing, onRunAnalysis }: {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <button
          ...
          className="rounded-[var(--radius-button)] bg-primary px-6 py-2 text-sm font-medium text-primary-foreground transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-soft-drop)]"
        >
      ...
      <ul className="space-y-2">
        {items.map((item, i) => (
          <li key={i} className="flex items-start text-sm">
            <Badge variant="secondary" className="mr-2 mt-0.5 h-5 w-5 rounded-full p-0 text-[10px]">
              {i + 1}
            </Badge>
            {item}
          </li>
